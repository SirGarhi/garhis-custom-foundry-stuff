...
### Likely to become Required in a future update
...
* Item Macro - Needed to examine and change the Item Macros on the items. Everything will function fine without it though.
  - https://foundryvtt.com/packages/itemacro
* DFreds Effects Panel - Provides a very convenient UI to see and end effects
  - https://foundryvtt.com/packages/dfreds-effects-panel
* Automated Animations - Most of the items are set up to use animations through AA.
  - https://foundryvtt.com/packages/autoanimations
* Compendium Folders - Without this the Items Compendium will be a mess. It should still function though.
...
### Workflow Settings
#### Workflow Tab
* Auto apply item effects - Set to any of the three "Apply Effects" options
...
Some items really on and assume custom effects exist with specific names. Those effects are listed here.
* Displacement - Applied by Cloak of Displacement item at the start of the cloak owner's turn. Should have a Special Duration of "is Damaged" and an effect with "flags.midi-qol.grants.disadvantage.attack.all"
* Thrumming - Applied by the Booming Blade Item Macro. Should have a duration of 1 round and a Special Duration of "Moved." The effect should be "macro.execute.GM" with an Effect Value of "GG_ThrummingExplosion"
...
### Testing Dummies 
Various actors for use in developing and testing new items and effects, not meant for use in games
...
* **Reckless Attack** - Applies two effects when used. The first grants advantage on melee attacks and expires at the end of the turn, the second grants advantage for all incoming attacks and expires at the start of the caster's next turn. The Convenient Effect approach has the advantage on attacks made by the caster persist as well, when it's not supposed to.
...
* **Guardian Petal Swarm** - Ability that rolls 1d6 to determine what to deduct from the triggering attack roll. Set up to not deal any damage so it doesn't matter who/what is targeted. Not flagged as a Reaction Ability but instead has Limited Uses that get recharged each turn while Blooming Rose Vortex is active.
...
* **Wolf Spirit Rage** - Creates an aura that forces enemies within range to grant advantage on melee attacks against them. This aura starts disabled and will dynamically turn on at the end of the Barbarian's turn, then turn back off at the start of their turn so the Barbarian themselves doesn't benefit from the aura effect on targets. Edge Case: Opportunity Attacks, or other attacks taken out of turn will benefit from the Wolf Aura.
...
* **Eagle Spirit Rage** - No Mechanical benefit, purely a tracker that the ability is active.
...
#### Sapphie
* **Lilting Performance** - When used adds 1 to the number of uses of Bardic Inspiration. Does not go over maximum but also doesn't inform the caster if it was "wasted"
...
* **Elemental Payload/Infused** - Applies a typed damage bonus to each shot with the weapon using the ammo.
...
* **Ammo Pouch** - simply an item to track which types of ammo are available.
